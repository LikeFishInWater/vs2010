...
bool CComm::Initial()
{
	...
	for (int i = 0; i < 12; i++)
	{
		TX_MOD_SYM[i] = new CComplex[MSG_LEN];
	}
	RX_MOD_SYM = new CComplex*[12];
	for (int i = 0; i < 12; i++)
	{
		RX_MOD_SYM[i] = new CComplex[MSG_LEN];
	}

	RX_LLR = new double*[16];
	for (int i = 0; i < 16; i++)
	{
		RX_LLR[i] = new double[MSG_LEN];
	}
	RX_DECODE = new int*[16];
	for (int i = 0; i < 16; i++)
	{
		RX_DECODE[i] = new int[MSG_LEN];
	}
...
int CComm::Spread(void)
{
	for (int i = 0; i < 12; i++)
	{
		for (int j = 0; j < MSG_LEN; j++)
		{
			TX_MOD_SYM[i][j].Real = TX_MSG[LdsStrH[i][0]][j] * ReLdsStr[i][LdsStrH[i][0]] + TX_MSG[LdsStrH[i][1]][j] * ReLdsStr[i][LdsStrH[i][1]] + TX_MSG[LdsStrH[i][2]][j] * ReLdsStr[i][LdsStrH[i][2]] + TX_MSG[LdsStrH[i][3]][j] * ReLdsStr[i][LdsStrH[i][3]];
			TX_MOD_SYM[i][j].Image = TX_MSG[LdsStrH[i][0]][j] * ImLdsStr[i][LdsStrH[i][0]] + TX_MSG[LdsStrH[i][1]][j] * ImLdsStr[i][LdsStrH[i][1]] + TX_MSG[LdsStrH[i][2]][j] * ImLdsStr[i][LdsStrH[i][2]] + TX_MSG[LdsStrH[i][3]][j] * ImLdsStr[i][LdsStrH[i][3]];
		}
	}
	return 1;
}
